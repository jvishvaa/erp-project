...
const AdminEditCreateBlogs = () => {
  ...
  // let branchIdss = selectedBranch.map((obj, index) => obj?.branch_name).join(', ');
  ...
  useEffect(() => {
    if (history?.location?.pathname === '/blog/admineditcreateblogs') {
      handlePreview(history?.location?.state?.data?.id);
      console.log(history?.location?.state?.data,"from");
      ...
      // setSubmission(history?.location?.state?.data?.submission_date);
