...
export default function Enviroment(props) {
  ...
  const fetchPostLike = (list) => {
    axiosInstance
      .get(`${endpoints.discussionForum.postLike}?post=${list.id}&&type=1`)
      .then((res) => {
              ...
              <CardContent style={{ backgroundColor: index % 2 === 0 ? '#FEE4D4' : '#DDEF96' }}>
                  <Typography className="user-info" color="textSecondary">
                      <div className="user-name-icon">
                          ...
                          <span style={{ marginLeft: -34, marginTop: 10, fontSize: 14, color: '#042955', fontWeight: 600 }}>
{list.post_by.first_name} 
{' '}
{list.post_by.last_name}
</span>
              ...
              <CardActions>
                  <div className="env-icns tooltip" style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <IconButton
onClick={(e) => handleClick(e, list, index)} 
            ...
            >
              ...
              />
                          {showPeriodIndex === index && likeList ? (
<Popper id={index} open={open} anchorEl={anchorEl} transition className="tool-tip" >
                                {({ TransitionProps }) => (
