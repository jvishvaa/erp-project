...
const arr = [
  ...
  {
    id: 2,
    subjectName: 'Science Quiz 1 ',
    belowThreshold: '100',
    avgMarks: '90%',
  },
  {
    id: 3,
    subjectName: 'Language Quiz 1 ',
    belowThreshold: '100',
    avgMarks: '90%',
  },
  {
    id: 4,
    subjectName: 'Combined Test 1',
    belowThreshold: '100',
    avgMarks: '90%',
  },
  {
    id: 1,
    subjectName: 'Math Quiz 2',
    belowThreshold: '100',
    avgMarks: '90%',
  },
];
const SubjectWiseDetails = (props) => {
  ...
  const getSubjectWiseStat = (params = {}) => {
    setLoading(true);
    axiosInstance
      .get(`${endpoints.ownerDashboard.subjectWiseClassParticipationStudentReportStat}`, {
        params: { ...params },
        headers: {
      ...
      .then((res) => {
        console.log(res);
        setAllSubjectWiseStat(res.data.result.data);
                        ...
                        allSubjectWiseStat.map((each, index) => {
                          return (
                            <TableRow key={index}>
                              ...
                              <TableCell>{each.below_threshold}</TableCell>
                              <TableCell className={clsx(classes.tableCellRightAlign)}>
                                {each.avg_score}
                              </TableCell>
                              <TableCell className={clsx(classes.tableCellRightAlign)}>
                                <IconButton
                                  ...
                                  onClick={() =>
                                    history.push({
                                      pathname: `/student-report/classparticipation-student-wise/${branchId}/${gradeId}/${sectionId}/${each.subject_id}`,
                                      state: {
