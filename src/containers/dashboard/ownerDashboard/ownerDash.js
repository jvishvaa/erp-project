...
import { Grid, MenuItem, FormControl, InputLabel, Select, Card, IconButton, Typography } from '@material-ui/core';
...
const OwnerDashboard = () => {
    ...
    const getrecenttransaction = () => {
        axios.get(`${endpoints.ownerDashboard.getRecentTransaction}?academic_year=${selectedAcademicYear?.session_year}&branch=${selectedBranchId}&date=${date}`)
        .then((res) => {
    ...
    const getFinanceReport = (params) => {
        axios.get(`${endpoints.ownerDashboard.getFinanceDetails}?academic_year=${selectedAcademicYear?.session_year}&branch=${selectedBranchId}`)
            .then((res) => {
                console.log(res, "finan");
                setFinanceData(res.data)
                // setRoleWiseAttendance(res.data.result)
    ...
    return (
        <Grid container spacing={2}>
            ...
            >
                <Grid item md={2}>
                    <Typography style={{ fontSize: '32px', fontWeight: 'bolder' }}> Dashboard </Typography>
                </Grid>
            ...
            <Grid container xs={12} sm={8} md={8} spacing={1}>
                ...
                <FinanceOwnerDashboard roleWiseAttendance={roleWiseAttendance} financeData={financeData} 
                studentAttendance={studentAttendance} 
                studentAttendanceOverview={studentAttendanceOverview} avgTest={avgTest} branchList={branchList} 
                currBranch={currBranch} staffOverAll={staffOverAll} 
                moduleId={moduleId} handleTodayAttendance={handleTodayAttendance} handleFeeRefresh={handleFeeRefresh}
                 handleAcadRefresh={handleAcadRefresh} 
                handlestaffOverViewRefresh={handlestaffOverViewRefresh} 
                todayCounter={todayCounter}  acadCounter={acadCounter} 
