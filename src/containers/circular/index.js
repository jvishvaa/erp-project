...
//for edit circular data
    ...
    const handlePeriodList = (grade,branch,section) => {
        ...
        axiosInstance.get(`${endpoints.circular.circularList}?is_superuser=True&branch=${branch.id}&grade=${grade.grade_id}&section=${section.id}&page=${page}&page_size=${limit}`
        )
        .then(result => {
    ...
    useEffect(() => {
        if (page && grade && branch && section )
            handlePeriodList(grade, branch, section)
    }, [page]);
