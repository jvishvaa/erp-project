  ...
  ({
    ...
    useEffect(() => {
      ...
      if (history?.location?.state?.currentHomework && teacherModuleId) {
        ...
        setSectionDisplay(currentHomework?.section_id);
        setGradeDisplay(currentHomework?.grade_id);
        setSectionMap(currentHomework?.section_mapping);
        setStartDate(moment().subtract(2, 'd').format('YYYY-MM-DD'));
        setEndDate(moment().add(4, 'd').format('YYYY-MM-DD'));
        getSection(currentHomework?.grade_id);
        formRef.current.setFieldsValue({
          grade: currentHomework?.grade_name,
          section: currentHomework?.section_name,
        });
      }
    ...
    const getTeacherListApi = async () => {
      ...
      try {
        ...
        const result = await axiosInstance.get(
          `${endpoints.coordinatorTeacherHomeworkApi.getAllTeacherList}?section_mapping=${sectionMap}&module_id=${teacherModuleId}`,
          {
            headers: {
    ...
    const getGrade = () => {
      axiosInstance
        .get(
          `${endpoints.academics.grades}?session_year=${selectedAcademicYear?.id}&branch_id=${selectedBranch?.branch?.id}&module_id=${teacherModuleId}`
        )
        .then((result) => {
          if (result.status === 200) {
      ...
      <>
        ...
        <Layout>
          {activeView == false ? (
              ...
              <div className='message_log_white_wrapper'>
                ...
                <div className='create_group_filter_container'>
                  ...
                  >
                    ...
                    >
                      {endDate != undefined ? (
                        <div>{`Date Range Selected : ${startDate} TO ${endDate}`}</div>
                      ) : (
                        ''
                      )}
                    </div>
                  ...
                  <div>
                    <>
                      {fetchingTeacherHomework ? (
                        ...
                        <Grid xs={12} item className='home-work-grid'>
                          {homeworkCols?.length > 0 ? (
                            <WeeklyTable
                              homeworkCols={homeworkCols}
                              homeworkRows={homeworkRows}
                              branch={selectedBranch?.branch?.id}
                              grade={gradeDisplay}
                              sectionMapping={sectionMap}
                              sectionId={sectionDisplay}
                              teacherid={selectedCoTeacherOptValue}
                              moduleId={teacherModuleId}
                              startDate={startDate}
                              endDate={endDate}
                              isTeacher={isTeacher}
            ...
            <div>
              {activeView === true && (
                <ViewHomeworkNew homework={viewHomework} onClose={handleDoneEvaluate} />
              )}
            </div>
          )}
        </Layout>
