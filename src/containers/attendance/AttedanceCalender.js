  ...
  const getStudentRange = () => {
    if (counter === 2) {
      ...
      axiosInstance
        ...
        axiosInstance
        .get(
          `${endpoints.academics.getEvents}?session_year_id=${sessionId?.id}&start_date=${startDate}&end_date=${endDate}&grade=${gradesId}&level=${user_level}`
        )
        .then((res) => {
