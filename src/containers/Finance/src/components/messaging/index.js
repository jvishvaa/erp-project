...
class Communicate extends Component {
  ...
  getGeneralDiaries = (path) => {
    this.setState({ loading: true })
    axios.get(`${urls.GeneralDiary}${path}`, {
      headers: {
  ...
  componentDidMount () {
    ...
    let path = ''
    if (this.role === 'Admin' || this.role === 'MIS' || this.role === 'CFO') {
      path += `?page_number=${this.state.currentPage}`
      this.getGeneralDiaries(path)
      ...
      let branchId = academicProfile.branch_id
    ...
    if (this.role === 'Admin' || this.role === 'MIS' || this.role === 'CFO') {
      if (target.scrollTop + target.clientHeight >= target.scrollHeight) {
        ...
        if (selectorData.section_mapping_id) {
          ...
          path += `?page_number=${currentPage + 1}&secmap_id=${sectionId}`
          ...
          path += `?page_number=${currentPage + 1}&branch_id=${branchId}`
        } else {
          path += `?page_number=${currentPage + 1}`
        }
        axios.get(`${urls.GeneralDiary}${path}`, {
          headers: {
            Authorization: 'Bearer ' + this.props.user,
  ...
  handleDateChange (update) {
    let path = ''
    this.setState({ selectedDate: update.format('YYYY-MM-DD'), currentPage: 1 }, () => {
      path += `?page_number=${this.state.currentPage}&student_id=${this.studentId}&date=${this.state.selectedDate}`
      this.getGeneralDiaries(path)
    })
  }
  ...
  render () {
    ...
    return (
      <React.Fragment>
        ...
        </Toolbar>{this.state.loading ? <InternalPageStatus label={'Loading...'} />
            ...
            TITLE :
             ...
             MESSAGE :
                    ...
                    <Typography className='card-text' variant='body1'>
                      ...
                      <Chip
                        size='medium'
                        label={moment(message.uploaded_time).format('DD-MM-YYYY') || 'NO UPLOADED TIME'}
