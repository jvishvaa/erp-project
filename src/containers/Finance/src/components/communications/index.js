...
import Communicating from '../communicating'
  ...
  render () {
    ...
    return <React.Fragment>
      {this.role === 'Teacher' || this.role === 'Admin' || this.role === 'Principal' || this.role === 'LeadTeacher' || this.role === 'AcademicCoordinator'
        ...
        : ''}
      <Communicating alert={this.props.alert} onCloseMessage={() => this.setState({ newMessage: false })} newMessage={this.state.newMessage} />
      <Grid item xs={2}>
        {/* <ListItem button
          onOut
          onClick={(e) => {
