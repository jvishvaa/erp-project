...
class ParticipantReport extends Component {
  constructor () {
    super()
    this.state = {
      date: new Date(),
  ...
  participantReportDateWise = (state, pageSize) => {
    this.setState({ showText: true })
    let date = this.state.date ? moment(this.state.date).format('DD-MM-YYYY') : null
    pageSize = pageSize || this.state.pageSize

    axios.get(`${urls.participantReport + '?date=' + date + '&page_number=' + (state && state.page ? state.page + 1 : 1) + '&page_size=' + pageSize}`, {
      headers: {
      ...
      .then(res => {
        this.setState({
          participantReportDownload: res.data.urls,
          showText: true,
          loading: false
        })
      })
  ...
  getDateClassReport (url) {
    let part = (url.split('/')[url.split('/').length - 1]).replace(' participant-Report-', '')
  ...
  scrollHandle = (event, state, pageSize) => {
    ...
    if (target.scrollTop + target.clientHeight >= target.scrollHeight) {
      axios.get(`${urls.participantReport + '?date=' + date + '&page_number=' + (currentPage + 1) + '&page_size=' + pageSize}`, {
        headers: {
          Authorization: 'Bearer ' + this.props.user
          // 'Content-Type': 'application/json'
        }
        ...
        .then(res => {
          if (res.data.urls.length) {
            this.setState({
              ...
              participantReportDownload: [...this.state.participantReportDownload, ...res.data.urls]
            })
          }
        })
    }
  ...
  render () {
    console.log(this.state.participantReportDownload, 'par')
    const { participantReportDownload } = this.state
    ...
    return (
      <React.Fragment>
        <Grid style={{ 'font-size': '16px', padding: 10 }} >
          ...
          />
              &nbsp; &nbsp;
          <Button variant='contained' size='small' style={{ padding: 10 }} color='primary' onClick={this.participantReportDateWise}>
            FILTER
          </Button>
        </Grid>
        <div className='class-container' onScroll={this.scrollHandle}>
          {participantReportDownload && participantReportDownload.length > 0 && participantReportDownload.map(classReport => {
            return (
...
export default connect(mapStateToProps, null)(
  withStyles(styles)(withRouter(ParticipantReport)))
