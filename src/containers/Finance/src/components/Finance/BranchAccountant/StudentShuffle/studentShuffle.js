...
import { withStyles, Grid, Button, Paper, Table, TableBody, TableCell, TableHead, TableRow, TextField } from '@material-ui/core/'
              ...
              <TableBody>
                {studentShuffle.map(row => (
                  <TableRow>
                    ...
                    <TableCell align='right'>{row.branch_from && row.branch_from.branch_name ? row.branch_from.branch_name : ''}</TableCell>
                    <TableCell align='right'>{row.grade_from && row.grade_from.grade ? row.grade_from.grade : ''}</TableCell>
                    <TableCell align='right'>{row.section_from && row.section_from.section_name ? row.section_from.section_name : ''}</TableCell>
                    <TableCell align='right'>{row.branch_to && row.branch_to.branch_name ? row.branch_to.branch_name : ''}</TableCell>
                    <TableCell align='right'>{row.shuffle_initiated_by && row.shuffle_initiated_by.first_name ? row.shuffle_initiated_by.first_name : null}</TableCell>
                    <TableCell align='right'>{row.shuffle_initiation_date ? row.shuffle_initiation_date : ''}</TableCell>
                    <TableCell align='right'><div style={{ width: 150 }}>{row.reason ? row.reason : ''}</div></TableCell>
                    <TableCell align='right'>{row.to_approve_status ? row.to_approve_status : ''}</TableCell>
                    ...
                    <TableCell align='right'><Button onClick={() => { approveShufflingHandler(row.id) }} disabled={!accReasonToApprove[row.id]} className={classes.approve}>Approve</Button></TableCell>
                    <TableCell align='right'><Button onClick={() => { rejectShufflingHandler(row.id) }} disabled={!accReasonToApprove[row.id]} className={classes.reject}>Reject</Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
        ...
        <div>
          <Paper className={classes.root}>
            <Table className={classes.table}>
              ...
              <TableBody>
                {studentShuffle && studentShuffle.map(row => (
                  <TableRow>
                    <TableCell align='right'>{row.erp ? row.erp : ''}</TableCell>
                    <TableCell align='right'>{row.student && row.student.name ? row.student.name : ''}</TableCell>
                    <TableCell align='right'>{row.grade_from && row.grade_from.grade ? row.grade_from.grade : ''}</TableCell>
                    <TableCell align='right'>{row.section_from && row.section_from.section_name ? row.section_from.section_name : ''}</TableCell>
                    <TableCell align='right'>{row.branch_to && row.branch_to.branch_name ? row.branch_to.branch_name : ''}</TableCell>
                    <TableCell align='right'>{row.shuffle_initiated_by && row.shuffle_initiated_by.first_name ? row.shuffle_initiated_by.first_name : ''}</TableCell>
                    <TableCell align='right'>{row.shuffle_initiation_date ? row.shuffle_initiation_date : ''}</TableCell>
                    <TableCell align='right'><div style={{ width: 120 }}>{row.reason ? row.reason : ''}</div></TableCell>
                    <TableCell align='right'>{row.admin_approve_status ? row.admin_approve_status : ''}</TableCell>
                    <TableCell align='right'>{row.to_approve_status ? row.to_approve_status : ''}</TableCell>
                    <TableCell align='right'>{row.to_approve_status_updated_by ? row.to_approve_status_updated_by.first_name : ''}</TableCell>
                    <TableCell align='right'>{row.to_approve_status_remarks ? row.to_approve_status_remarks : ''}</TableCell>
                    <TableCell align='right'>{row.to_approve_status_date ? row.to_approve_status_date : ''}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
    ...
    <React.Fragment>
      <Grid container direction='row' justify='flex-end' spacing={1}>
        <Grid item className={classes.item} xs={3}>
          <Button variant='contained' onClick={requestHandler} className={classes.btn}>Request Shuffle</Button>
        </Grid>
      </Grid>
...
export default connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(withRouter(StudentShuffle)))
