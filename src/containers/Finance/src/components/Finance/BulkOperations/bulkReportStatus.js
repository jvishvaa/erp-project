...
const BulkReportStatus = ({
  ...
  useLayoutEffect(() => {
    ...
    if (role === 'FinanceAdmin' || role === 'SuperUser') {
      setIsAdmin(true);
    }
  }, []);
                  ...
                  ? statusList.results.map((row) => {
                      return (
                        <TableRow>
                          ...
                          <TableCell align='center'>
                            {row.download_file && row.download_file.length ? (
                              <a href={row.download_file} target='_blank'>
                                {row.download_file.substr(
                                  row.download_file.lastIndexOf('/') + 1
                                )}
                              </a>
                            ) : (
                              'NA'
              ...
              <TableFooter>
                <TableRow>
                  <TablePagination
                    ...
                    labelDisplayedRows={() =>
                      `${page + 1} of ${Math.ceil(
                        +statusList.count / (+rowsPerPage || 10)
                      )}`
                    }
  ...
  return (
    <Layout>
      <React.Fragment>
        ...
        {statusList && statusList.results && statusList.results.length
          ? getStatusHandler()
          : null}
...
const mapDispatchToProps = (dispatch) => ({
  ...
  fetchBulkReportStatus: (
    ...
    dispatch(
      actionTypes.fetchBulkReportStatus({
        sessionYear,
        branch,
        reportType,
        isAdmin,
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withStyles(styles)(withRouter(BulkReportStatus)));
