...
import { CustomAudioPlayer } from '../../../ui'
import Timer from './timer'
import { getSparseDate, secondsToTime } from '../../../utils'
...
const AudioPlayerWrapper = ({
  ...
  useEffect(() => {
    ...
    if (timedStart && dateToStart && timeToStart) {
      ...
      if (diff === 0 && diff > -1) {
        ...
        const dateToActivate = moment([startYear, startMonth - 1, startDate, hm[0], hm[1]])
  ...
  const generateToken = () => {
    ...
    axios.post(`${urls.GeneratePUblicToken}`, body, {
      headers: {
  ...
  const handleCopyLink = () => {
    navigator.clipboard.writeText(`${window.location.origin}/public/orchadio/${convertIdToBase64(radioProgramId)}/${publicToken}`)
  }
      ...
      <Grid container alignItems='center' justify='center'>
        ...
        >
          ...
          <Card
            ...
            <CardHeader title='Share with'
              action={
                <IconButton>
                  <CloseButton onClick={(e) => handleClickClose()} />
                </IconButton>
              }
            ...
            <CardContent className='orchadio__socialicons--container'>
              <div>
                <FacebookShareButton style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }} url={`${window.location.origin}/public/orchadio/${convertIdToBase64(radioProgramId)}/${publicToken}`}>
                  <FacebookIcon size={32} round />
                  <span style={{ marginLeft: 20, textTransform: 'uppercase', fontSize: 18 }}>Facebook</span>
                </FacebookShareButton>
                <br />
              </div>
              <div>
                <WhatsappShareButton style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }} url={`${window.location.origin}/public/orchadio/${convertIdToBase64(radioProgramId)}/${publicToken}`} >
                  <WhatsappIcon size={32} round />
                  <span style={{ marginLeft: 20, textTransform: 'uppercase', fontSize: 18 }}>Whatsapp</span>
                </WhatsappShareButton>
              </div>
              <div>
                <Button style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }} onClick={(e) => handleCopyLink(e)}>
                  <FileCopyIcon size={32} />
                  <span style={{ marginLeft: 20, fontSize: 18 }}>Copy Link</span>
                </Button>
              </div>
            </CardContent>
        ...
        {/* {!isPublic && <Button style={{ float: 'right' }} color='primary' variant='outlined' onClick={e => handleShareLink(e)}><ShareIcon /> &nbsp;share</Button>} */}
